...
class WebhookModule {
   ...
   * @param {string} [config.secretKey] - Secret key for signature verification
   */
    ...
    return async (req, res, next) => {
      try {
        ...
        if (verifySig && secretKey) {
          const signature = req.headers['x-zalo-signature'] || req.headers['x-zalo-signature'] || '';
          if (!this.verifySignature(signature, rawBody, secretKey)) {
        ...
        // Store event on req for downstream use
        req.zaloEvent = event;
        next();
      } catch (error) {
        // Log error but still respond 200 to avoid Zalo retries
